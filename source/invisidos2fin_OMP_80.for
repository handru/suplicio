...
c
      parameter (maxir=81,maxio=81,maxip=11,maxnk=2001,
...
c     internal files de Fortran
      character test1(maxro)*150,test2(maxro)*150,gamastr(maxro)*70,
...
c                      
      common test1,test2,gamastr,circostr,subrout,salida2out
c
...
c
c
      open(unit=5, file='entvis2f_80.in')
...
!      write(15,8)indice,ncapa
      ...
      do 770 in3=1,kult+1
       write(33,'(a137)')test1(in3)  !Escribe el archivo integ1.txt de estela
       write(39,'(a137)')test2(in3)  !Escribe el archivo integ2.txt de estela
...
c     &             dfi*fz(ik)/denom(ik)/ctte,valx,valy,valz
      write(test1(ik),44) ik,re(ir,ik)/rt,ze(ir,ik)/rt,fi(ir,ik),
...
c     &             ciex(npa,nhl),ciey(npa,nhl),ciez(npa,nhl)
      write(test1(ik+1),44)ik,re(ir,ik)/rt,ze(ir,ik)/rt,fi(ir,ik),
     &             dista(ik)/rt,denom(ik),1./denom(ik),
     &             dfi*fx(ik)/denom(ik)/ctte,
     &             dfi*fy(ik)/denom(ik)/ctte,
...
c     &             dfi*fy(ik)/denom(ik)/ctte,
c     &             dfi*fz(ik)/denom(ik)/ctte,valx,valy,valz
      write(test2(ik),44) ik,re(ir,ik)/rt,ze(ir,ik)/rt,fi(ir,ik),
     &             dista(ik)/rt,denom(ik),1./denom(ik),
     &             dfi*fx(ik)/denom(ik)/ctte,
     &             dfi*fy(ik)/denom(ik)/ctte,
     &             dfi*fz(ik)/denom(ik)/ctte,valx,valy,valz
...
c     &             dfi*fz(ik)/denom(ik)/ctte,
c     &             ciex(npa,nhl),ciey(npa,nhl),ciez(npa,nhl)
      write(test2(ik+1),44)ik,re(ir,ik)/rt,ze(ir,ik)/rt,fi(ir,ik),
     &             dista(ik)/rt,denom(ik),1./denom(ik),
     &             dfi*fx(ik)/denom(ik)/ctte,
     &             dfi*fy(ik)/denom(ik)/ctte,
     &             dfi*fz(ik)/denom(ik)/ctte,
...
c
! -------------------------------------------------------
! Test4 y Test5 sirven para reemplazar a los files temporales coefg.tmp y
! cindg.tmp. incoeg es el indice para recorrer test5, ya que se incrementa
! independiente del loop interno que genera los datos de test5
      real*8 test4(npan),test5(npan*npan)
      integer incoefg
! ------------------------------------
      integer tam
      character outstd(50)*100
...
c
  927 continue
      ...
      test5(incoefg)=coefg ! utilizacion de 1 array
      incoefg=incoefg+1
    2 continue 
c                    
...
c      
     ...
     &              UU*vnz(npa))
      ...
      test4(npa)=cindg ! escritura en array 
    1 continue
c
      !rewind(40)                   
      !rewind(41)                   
...
c
      call solgauss(npan,gamma,test5,test4)  
c                  
c     Tercera parte:
c
...
c-----------------------------------------------------------------------
c
      subroutine solgauss(npan,gama,array1,array2)
c
...
c
      ...
      real*8 array1(mxro*mxro),array2(mxro)
c
c.......................................................................
c
      m=npan+1
